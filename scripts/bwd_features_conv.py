...
if __name__ == "__main__":
    ...
    tb_fils = [datdir + 'merg_%s_4km-pixel.nc4' %(x) for x in date_range ]
    tb_ds = xr.open_mfdataset(tb_fils).sel(lon=slice(minlon,maxlon),lat=slice(minlat,maxlat)).squeeze()
    ...
    for iday, day in enumerate(np.delete(days,-1)):
        ...
        briT = tb.sel(time=slice(prvday,day)).mean(dim='time')
        print(briT.min,briT.max)
        ...
        briT = briT.sel(lon=slice(mnlon,mxlon),lat=slice(mnlat,mxlat))
        print(briT)
        ...
        cf = ax.contourf(briT.lon,briT.lat,briT.values-273.15,levels=tlevs,
                    cmap = 'rainbow_r',zorder=20,extend='min')
    ...
    plt.savefig('conv_env_%s_%s.eps' %(date,cyclone))
