...
def _get_2d_trajectories(traj_array):
    """
    """
    ntra, ntime = traj_array["lon"].shape
    trajectories = np.empty((ntra,ntime,3))
...
def _get_2d_trajxy(traj_array):
    ntra, ntime = traj_array.shape
    trajectories = np.empty((ntra,ntime,3))
    
    #2d_latlon = np.column_stack((traj_array['lat'],traj_array['lon']))
    convert = utm.from_latlon(traj_array['lat'],traj_array['lon'])
    trajectories[:,:,0] = np.array(convert[0])
    trajectories[:,:,1] = np.array(convert[1])
    return trajectories
