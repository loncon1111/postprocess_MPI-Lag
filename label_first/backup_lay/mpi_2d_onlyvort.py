...
ref_lev = 500   ## reference level


for icase in range(ndate+1):
    # Read label csv file
    df = pd.read_csv("../../CLU_VORT/labels/labellist2d_%s_%s_vort.csv" %(date,int(ref_lev)))
#    df = pd.read_csv("labellist2d_%s.csv" %date)
    ...
    for irow in range(ista,iend):
        ...
        if check_equal_lon == False and check_equal_lat == False and npoints < 1000 :
            ...
            try:
                min_owlag = np.nanmin([x for x in ow_lag1[ilev,hull_ilat,hull_ilon] if x != 0])
    ...
    if myrank == 0:
        ...
        data[ista:iend] = hull_data[ista:iend]
        ...
        for irank in range(1,nprocs):
            ...
            print("I received information from ",irank)
        ...
        df1["label"] = df["label"]
        df1["value"] = df["value"]
        df1.to_csv("label_list2d_%s_%s_vort.csv" %(date,int(ref_lev)))
...
#        df1.to_csv("label_list2d_%s_%s.csv" %(date,int(ref_lev)))
    ...
    data, hull_data = 2*[None]; ow_lag1 = None
